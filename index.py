...
def forgot_password(data):
	try:
		...
		# execute SQL query
		cursor.execute("USE main_db")
		cursor.execute("SELECT Id FROM Users WHERE Email='" + str(data["Email"]) + "'")
		# fetch a single row using
		result = cursor.fetchone()
	except Exception as e:
		db.close()
	...
	if cursor.rowcount == 1:
		try:
			#NOTICE: if database changes, the index result[2] may also change. [2] is the password field. a better solution may be neccessary?
			send_email(str(data["Email"]), '<html><body><h1>Your password is: ' + result[2]  + '</h1></body></html>')
		except Exception as e:
